...
import { format, addDays, startOfToday, parseISO, subDays, isAfter } from 'date-fns';
...
const BookAppointment = () => {
  ...
  const filteredDoctors = doctors.filter(doc => {
    ...
    // Compare department IDs (as string to be safe)
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await api.post(API_ENDPOINTS.PATIENT.BOOK_APPOINTMENT, {
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-secondary-500 dark:text-secondary-400 mt-1">
          Schedule a visit with our specialists at {activeClinic?.name || 'our clinic'}.
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-secondary-900 rounded-lg border border-secondary-200 dark:border-secondary-800 overflow-hidden">
        ...
        <div className="bg-secondary-50 dark:bg-secondary-900 border-b border-secondary-200 dark:border-secondary-800 p-4">
          <div className="flex items-center justify-center space-x-4">
            ...
            <div className={`h-1 w-8 sm:w-12 ${step >= 2 ? 'bg-primary-500/50' : 'bg-secondary-200 dark:bg-secondary-700'}`}></div>
            ...
            <div className={`h-1 w-8 sm:w-12 ${step >= 3 ? 'bg-primary-500/50' : 'bg-secondary-200 dark:bg-secondary-700'}`}></div>
            ...
            <div className="space-y-6">
              {fetchError && (
                <div className="p-4 bg-red-50 text-red-700 rounded-md flex items-center">
                   <AlertCircle className="w-5 h-5 mr-2"/> {fetchError}
                </div>
              )}
              ...
              <div>
                ...
                >
                  <option value="" className="bg-white dark:bg-secondary-900 text-secondary-900 dark:text-white">
                    {isLoadingDoctors ? 'Loading...' : '-- Choose Department --'}
                  </option>
                  {departments.map(dept => (
                    <option key={dept.id} value={dept.id} className="bg-white dark:bg-secondary-900 text-secondary-900 dark:text-white">{dept.name}</option>
                  ))}
                </select>
              ...
              <div>
                ...
                {isLoadingDoctors ? (
                  <div className="text-center py-8 text-secondary-500">Loading doctors...</div>
                ) : filteredDoctors.length === 0 ? (
                  <div className="text-center py-8 text-secondary-500 border rounded-lg border-dashed">
                    {selectedDepartment ? 'No doctors available in this department.' : 'Please select a department first.'}
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {filteredDoctors.map((doc) => (
                      <div
                        ...
                        onClick={() => setValue('doctor', String(doc.id))}
                      >
                        <div className="flex items-center space-x-4">
                          <div className="h-12 w-12 rounded-full bg-secondary-100 dark:bg-secondary-700 flex items-center justify-center text-secondary-500 dark:text-secondary-400 overflow-hidden">
                             {doc.profile_photo ? (
                               <img src={`${import.meta.env.VITE_BACKEND_BASE_URL}/${doc.profile_photo}`} alt={doc.user.name} className="w-full h-full object-cover"/>
                             ) : (
                               <User className="w-6 h-6" />
                             )}
                          </div>
                          <div className="flex-1">
                            <h3 className="font-bold text-secondary-900 dark:text-white">Dr. {doc.user?.name || doc.name}</h3>
                            <p className="text-sm text-secondary-500 dark:text-secondary-400 mb-1">
                                {doc.department?.name || (Array.isArray(doc.specialization) ? doc.specialization.join(', ') : doc.specialization)}
                            </p>
                            
                            <div className="text-xs text-secondary-500 space-y-1">
                                <div className="flex items-center gap-2">
                                    <span className="font-medium text-secondary-700 dark:text-secondary-300">Room:</span> 
                                    {doc.consultation_room_number || 'N/A'} ({doc.consultation_floor || 'GF'})
                                </div>
                                
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="font-medium text-secondary-700 dark:text-secondary-300">Fee:</span>
                                    {doc.follow_up_fee && doc.follow_up_fee < doc.consultation_fee ? (
                                        <div className="flex items-center gap-2">
                                            <span className="line-through text-secondary-400 text-[10px]">${doc.consultation_fee}</span>
                                            <span className="text-green-600 font-bold">${doc.follow_up_fee}</span>
                                        </div>
                                    ) : (
                                        <span className="font-semibold text-secondary-900 dark:text-white">${doc.consultation_fee || '0'}</span>
                                    )}
                                </div>
                            </div>
                          </div>
          ...
          {step === 2 && (
            <div className="space-y-8">
              ...
              <div>
                ...
                {isLoadingSlots ? (
                   <div className="py-4 text-center text-secondary-500">Checking availability...</div>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                    {availableSlots.map((slot) => (
                      <button
                        key={slot.start}
                        type="button"
                        onClick={() => setValue('time', slot.start)}
              ...
              <div className="bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-500/20 rounded-lg p-6">
                ...
                <div className="mb-6 p-4 bg-white dark:bg-secondary-800 rounded-md border border-secondary-200 dark:border-secondary-700">
                  <div className="flex items-center justify-between mb-2">
                    <label className="block text-sm font-medium text-secondary-700 dark:text-secondary-300">Appointment Type</label>
                    {isCheckingHistory && <span className="text-xs text-primary-500 animate-pulse">Verifying history...</span>}
                  </div>
                  ...
                  <div className={`flex items-center justify-between p-3 rounded-lg border ${
                    ...
                    <div className="flex items-center space-x-3">
                      <div className={`p-2 rounded-full ${
                        watch('type') === 'follow_up' ? 'bg-green-100 text-green-600' : 'bg-primary-100 text-primary-600'
                      }`}>
                         {watch('type') === 'follow_up' ? <Clock className="w-5 h-5"/> : <CheckCircle className="w-5 h-5"/>}
                      </div>
                      <div>
                        <p className="font-bold text-secondary-900 dark:text-white">
                          {watch('type') === 'new' ? 'New Consultation' : 'Follow-up Visit'}
                        </p>
                    ...
                    <span className={`text-lg font-bold ${
                       watch('type') === 'follow_up' ? 'text-green-600' : 'text-primary-600'
                    }`}>
                      ${watch('type') === 'new' 
                          ? (getDoctorDetails(watch('doctor'))?.consultation_fee || 0)
                          : (getDoctorDetails(watch('doctor'))?.followup_fee || getDoctorDetails(watch('doctor'))?.consultation_fee || 0)
                      }
                    </span>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-secondary-900 dark:text-white">
                      Dr. {getDoctorDetails(watch('doctor'))?.user?.name || getDoctorDetails(watch('doctor'))?.name}
                    </span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between border-t border-secondary-200 dark:border-secondary-700 pt-2 mt-2">
                    <span className="text-secondary-900 dark:text-white font-bold">Estimated Fee:</span>
